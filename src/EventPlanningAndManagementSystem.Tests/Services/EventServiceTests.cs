...
namespace EventPlanningAndManagementSystem.Tests.Services
{
    public class EventServiceTests
    {
        ...
        [Fact]
        public async Task GetEventForEditAsync_ShouldReturnModel_WhenEventExistsAndOwnedByUser()
        {
            var db = GetDbContext();
            var userManager = GetUserManagerMock();
            ...
            var result = await service.GetEventForEditAsync("user-1", 1);
        ...
        [Fact]
        public async Task GetEventForEditAsync_ShouldReturnNull_WhenEventNotOwnedByUser()
        {
            var db = GetDbContext();
            var userManager = GetUserManagerMock();
            ...
            var result = await service.GetEventForEditAsync("user-2", 2);

            result.Should().BeNull();
        }

        [Fact]
        public async Task PersistUpdatedEventAsync_ShouldUpdateEvent_WhenAuthorized()
        {
            var db = GetDbContext();
            var userManager = GetUserManagerMock();
            ...
            var service = new EventService(db, userManager);
            var updatedModel = new EditEventViewModel
            {
                Id = 3,
                Name = "New Name",
                Description = "New",
            ...
            var result = await service.PersistUpdatedEventAsync("user-1", updatedModel);

            result.Should().BeTrue();
            var updated = await db.Events.FindAsync(3);
            updated!.Name.Should().Be("New Name");
        }

        [Fact]
        public async Task PersistUpdatedEventAsync_ShouldReturnFalse_WhenEventNotFound()
        {
            var db = GetDbContext();
            var userManager = GetUserManagerMock();
            var service = new EventService(db, userManager);
            ...
            var result = await service.PersistUpdatedEventAsync("user-1", model);
            result.Should().BeFalse();
        }

        [Fact]
        public async Task PrepareNewEventFormModelAsync_ShouldReturnModelWithSelectLists()
        {
            var db = GetDbContext();
            var userManager = GetUserManagerMock();

            db.Categories.Add(new Category { Id = 1, Name = "C1" });
            db.Locations.Add(new Location { Id = 1, Name = "L1" });
            await db.SaveChangesAsync();

            var service = new EventService(db, userManager);

            var model = await service.PrepareNewEventFormModelAsync();

            model.Categories.Should().HaveCount(1);
            model.Locations.Should().HaveCount(1);
        }

        [Fact]
        public async Task PrepareNewEventFormModelAsync_WithModel_ShouldPopulateSelectLists()
        {
            var db = GetDbContext();
            var userManager = GetUserManagerMock();
            ...
            var input = new AddEventInputModel { Name = "Input Test" };
            var model = await service.PrepareNewEventFormModelAsync(input);

            model.Categories.Should().HaveCount(1);
            model.Locations.Should().HaveCount(1);
            model.Name.Should().Be("Input Test");
        }

        [Fact]
        public async Task PrepareEditEventModelAsync_ShouldPopulateSelectLists()
        {
            var db = GetDbContext();
            var userManager = GetUserManagerMock();
            ...
            var model = new EditEventViewModel { Name = "Edit", CategoryId = 1, LocationId = 1 };
            var result = await service.PrepareEditEventModelAsync(model);
